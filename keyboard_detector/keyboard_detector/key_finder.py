...
class KeyFinder:
    def __init__(self, 
                 ...
                 cluster_epsilon = 50,
                 keys_pattern_file=os.path.join('.',"src","keyboard_detector",'keyboard_detector','clean_letters.json')):
    ...
    def original_coords(self, key, value, img):
        x, y = value['coords'][0], value['coords'][1] 
        x = x*(self.return_values[1] - self.return_values[0]) + self.return_values[0] 
        y = y*(self.return_values[3] - self.return_values[2]) + self.return_values[2] 
        coords = np.matrix([x,y])
        inverse = np.transpose(np.matrix(self.base2))
        coords = np.matmul(coords, inverse)
        
        x = coords[0,0] + np.mean(self.centers[:,0])
        y = coords[0,1] + np.mean(self.centers[:,1])
        x = x+ int(x<0)*img.shape[2]
        y = y+ int(y<0)*img.shape[1]
